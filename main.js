...
let thebrograssbar = document.getElementById('brograssbar')
let prograssbar = document.getElementById("progbar");
window.addEventListener("scroll", function () {
  ...
  if (this.scrollY <= 731) {
    ...
    let scaleValueimg = (0.2 / 731) * value;
    hero_img.style.transform = `translate(${466 - translateXValueimg}px, ${
      150 - translateYValueimg
    }px) scale(${scaleValueimg + 1})`;
    let translateXValuetext = 0.4254 * value;
    let translateYValuetext = 0.2188 * value;
    let scaleValuetext = (1 / 1462) * value;
    hero_text.style.transform = `translate(${
      translateXValuetext - 311.002
    }px, ${translateYValuetext - 160}px) scale(${scaleValuetext + 0.5})`;
  }
  ...
  if (this.scrollY > 10) {
    ...
    prograssbar.style.width = scrollPercentage + "%";
    thebrograssbar.style.width = 100+ "%";
  }
});
